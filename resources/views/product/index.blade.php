...
@section('content')
    <div class="container mx-auto">
        ...
        <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
            ...
            @if (count($tags) <= 0)
                <div class="mb-1 alert alert-info" role="alert">
                    <p style="color: lightsalmon">No Tag found, please Create a Tag first!</p>
                </div>
            @endif

            @if (count($categories) <= 0)
                <div class="mb-2 alert alert-info" role="alert">
                    <p style="color: lightsalmon">No Category found, please Create a Category first!</p>
                </div>
                <hr>
            @endif
                ...
                <div class="mt-1">
                    <select name="categories_id" id="categories"
                        ...
                        @foreach ($categories as $category)
                            <option class="bg-gray-500 hover:bg-gray-200" value="{{ $category->id }}">{{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mt-1">
                    @foreach ($tags as $tag)
                        <input type="checkbox" name="tags_id[]" value="{{ $tag->id }}"
                            class="my-1 mx-2 form-checkbox h-5 w-5 text-indigo-600">{{ $tag->name }}
                    @endforeach
                </div>
